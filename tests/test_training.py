...
def apply_mlflow_patches(func):
    """Decorator that applies all MLflow patches."""
    import functools
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        for p in reversed(MLFLOW_PATCHES):
            func = p(func)
...
class TestLoadDatasets:
    ...
    def test_file_not_found(self, tmp_path, monkeypatch):
        from sessionscout.model.dataset import load_datasets
        monkeypatch.setattr(cfg.paths, "sequences_parquet", tmp_path / "missing.parquet")
...
def test_train_logistic_regression(tmp_path, monkeypatch):
    ...
    with patch("mlflow.set_tracking_uri"), patch("mlflow.set_experiment"), \
         patch("mlflow.start_run", side_effect=fake_mlflow_run), \
         patch("mlflow.log_metrics"), patch("mlflow.log_params"):
...
def test_train_deep_model_lstm(tmp_path, monkeypatch):
    ...
    with patch("sessionscout.model.dataset.load_datasets",
               return_value=(train_ds, val_ds, test_ds)), \
         patch("sessionscout.model.dataset.make_dataloaders",
               return_value=(DataLoader(train_ds, batch_size=32),
        ...
        result = train_deep_model(
            SessionLSTM(), "lstm", "test-lstm", epochs=2, lr=1e-3
        )
...
def test_main_lr(tmp_path, monkeypatch):
    ...
    with patch("sys.argv", ["train", "--model", "lr"]), \
         patch("mlflow.set_tracking_uri"), patch("mlflow.set_experiment"), \
         patch("mlflow.start_run", side_effect=fake_mlflow_run), \
         patch("mlflow.log_metrics"), patch("mlflow.log_params"):
        main()
...
class TestEvaluate:
    ...
    def test_print_results_table(self, capsys):
        ...
        results = {
            ...
            "transformer": {"val_auc": 0.981, "test_auc": 0.984, "ap": 0.82, "p_at_500": 0.88},
        }
