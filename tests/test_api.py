...
def client():
    ...
    with patch("sessionscout.api.main.load_model", return_value=mock_model), \
...
class TestBatch:
    ...
    def test_batch_response_schema(self, client):
        resp = client.post("/api/v1/batch", json={
            "sessions": [{"session_id": "x", "sequence": [1, 2]}]
        })
        data = resp.json()
    ...
    def test_batch_total_matches_input(self, client):
        sessions = [
            {"session_id": f"s{i}", "sequence": [1, 2, 1]}
            for i in range(5)
        ]
    ...
    def test_batch_each_result_has_probability(self, client):
        resp    = client.post("/api/v1/batch", json={
            "sessions": [{"session_id": "t", "sequence": [1, 2]}]
        })
