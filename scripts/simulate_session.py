...
def load_session(session_id: str = None, converting: bool = False,
                 abandoning: bool = False) -> tuple:
    """
    ...
    if session_id:
        ...
        if len(row) == 0:
            ...
            print(f"Available sessions (sample): "
                  f"{df['session_id'].head(5).tolist()}")
            sys.exit(1)
        row = row.iloc[0]
    elif converting:
        ...
        candidates = df[
            (df["label"] == 1) &
            (df["n_carts"] > 0) &
            (df["seq_len"].between(5, 15))
        ]
        row = candidates.iloc[0] if len(candidates) > 0 else df[df["label"] == 1].iloc[0]
    elif abandoning:
        ...
        row = candidates.iloc[0] if len(candidates) > 0 else df[df["label"] == 0].iloc[0]
    else:
...
def simulate(session_id: str, tokens: list, label: int, delay: float = 0.8):
    ...
    for i, token in enumerate(tokens):
        ...
        elif token == cfg.vocab.view and i > 0 and tokens[i-1] in (
            cfg.vocab.gap_short, cfg.vocab.gap_long
        ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Simulate a live session scoring demo"
    )
    parser.add_argument("--session-id", type=str, default=None)
    parser.add_argument("--converting",  action="store_true",
                        help="Show a session that ended in purchase")
    parser.add_argument("--abandoning",  action="store_true",
                        help="Show a session that abandoned")
