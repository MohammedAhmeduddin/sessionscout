...
def train_logistic_regression():
    ...
    X_train, X_val, X_test, y_train, y_val, y_test, feature_cols = load_tabular_splits()
    ...
    with mlflow.start_run(run_name="logistic-regression"):
        ...
        coef_importance = sorted(
            zip(feature_cols, abs(model.coef_[0])), key=lambda x: x[1], reverse=True
        )[:5]
...
def train_xgboost():
    ...
    with mlflow.start_run(run_name="xgboost"):
        ...
        return {"model": "xgboost", "val_auc": val_auc, "test_auc": test_auc}
...
def train_deep_model(model, model_name, run_name, epochs=None, lr=None):
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        for epoch in range(1, epochs + 1):
            ...
            mlflow.log_metrics(
                {"train_loss": round(avg_loss, 4), "val_auc": round(val_auc, 4)},
                step=epoch,
            )
            logger.info(
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                mask = batch["attention_mask"].to(device)
                logits = model(ids, mask)
                test_logits.extend(torch.sigmoid(logits).cpu().tolist())
        ...
        return {"model": model_name, "val_auc": best_val_auc, "test_auc": test_auc}
